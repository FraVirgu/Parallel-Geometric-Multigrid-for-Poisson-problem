...
error_file = os.path.join("OUTPUT_RESULT", "errors_it_cg.txt")
...
def plot_linear(curves, output_path):
    ...
    plt.ylabel("Error")
    plt.title("Error Convergence (Linear Scale) per Iteration for Conjugate Gradient Method")
    plt.legend()
...
def plot_loglog(curves, output_path):
    ...
    plt.xscale('log')
    plt.yscale('log')
    plt.title("Error Convergence (Log-Log Scale) per Iteration for Conjugate Gradient Method")
    plt.legend()
    plt.grid(True, linestyle='--', linewidth=0.5)
    plt.tight_layout()
    plt.savefig(output_path, format='pdf')
...
# Linear plot
linear_path = os.path.join(plot_dir, "cg_error_curves_linear.pdf")
plot_linear(error_curves, linear_path)

# Log-log plot
loglog_path = os.path.join(plot_dir, "cg_error_curves_loglog.pdf")
